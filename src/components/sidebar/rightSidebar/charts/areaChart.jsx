...
function processArticlesData(articles) {
  if (!articles || articles.length === 0) {
    ...
    return [
      { year: "2018", microgravity: 45, radiation: 32, agriculture: 28, lifesupport: 21, other: 15 },
      { year: "2019", microgravity: 52, radiation: 41, agriculture: 35, lifesupport: 29, other: 18 },
      { year: "2020", microgravity: 68, radiation: 55, agriculture: 48, lifesupport: 42, other: 25 },
      { year: "2021", microgravity: 89, radiation: 73, agriculture: 61, lifesupport: 58, other: 34 },
      { year: "2022", microgravity: 112, radiation: 95, agriculture: 82, lifesupport: 71, other: 43 },
      { year: "2023", microgravity: 134, radiation: 118, agriculture: 105, lifesupport: 89, other: 55 },
      { year: "2024", microgravity: 156, radiation: 142, agriculture: 128, lifesupport: 114, other: 67 },
    ];
  }

  // Mapear tags a categorías conocidas
  const tagMapping = {
    'microgravity': ['microgravity', 'zero gravity', 'weightlessness', 'micro-gravity'],
    'radiation': ['radiation', 'cosmic rays', 'space radiation', 'solar', 'ionizing'],
    'agriculture': ['agriculture', 'plants', 'photosynthesis', 'crops', 'food', 'botanical'],
    'lifesupport': ['life support', 'oxygen', 'air', 'atmosphere', 'breathing', 'environmental'],
    'biology': ['biology', 'organisms', 'cells', 'genetics', 'dna', 'molecular', 'cellular'],
  ...
  articles.forEach(article => {
    ...
    (article.tags || []).forEach(tag => {
      ...
      yearCategoryData[year][category] = (yearCategoryData[year][category] || 0) + 1;
    });
  });
  ...
  const availableYears = Object.keys(yearCategoryData).map(y => parseInt(y)).sort();
  const minYear = availableYears.length > 0 ? Math.min(...availableYears) : 2018;
...
export function SpaceResearchChart({ articles = [] }) {
  // Procesar datos de artículos
  const chartData = useMemo(() => processArticlesData(articles), [articles]);
  ...
  const categories = useMemo(() => {
    ...
    const categoryTotals = keys.map(category => {
      const total = chartData.reduce((sum, yearData) => sum + (yearData[category] || 0), 0);
  ...
  const totalPublications = chartData.reduce((total, yearData) => {
    return total + Object.keys(yearData)
      .filter(key => key !== 'year')
            ...
            <defs>
              {categories.map(category => (
                <linearGradient key={category} id={`fill${category}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={categoryColors[category] || '#95A5A6'} stopOpacity={0.8} />
                  <stop offset="95%" stopColor={categoryColors[category] || '#95A5A6'} stopOpacity={0.1} />
                </linearGradient>
              ))}
            </defs>
      ...
      <div className="p-3 pt-0 mt-auto">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-white">
            {growthPercentage > 0 ? '+' : ''}{growthPercentage}% vs prev year
          </span>
        </div>
